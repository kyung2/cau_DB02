...
public abstract class KidsCafeDetailTask extends AsyncTask<String, Void, KidsCafeData> {
    //data={"user_id":19,"longitude":126.95,"latitude":37.50,"range":1}37.50,126.95
    private int id;

    protected KidsCafeDetailTask(int id) {
        this.id = id;
    }

    @Override
    protected KidsCafeData doInBackground(String... params) {
        InputStream inputStream;
        String strResult = "";
        String result = null;
        KidsCafeData item = null;
        try {
            URL url = new URL(NetworkManager.serverIP + "/kidsCafe/detail/");
            HttpURLConnection urlConnection = (HttpURLConnection) url.openConnection();
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("id", id);
            String strJson = "data=" + jsonObject.toString();
            Log.d("KidsCafeDetailTask", strJson);
            ...
            Log.d("KidsCafeDetailTask", strResult);
            JSONObject jsonObject1 = new JSONArray(new JSONObject(strResult).getString("kidscafe")).getJSONObject(0).getJSONObject("fields");
            ...
            Log.d("KidsCafeDetailTask",jsonObject1.getString("name"));// 데이터 위에에서 뽑으면 이렇게 나옴 (이건 name)임
        ...
        return KidsCafeData.getSampleKidsCafeData();
    }
}